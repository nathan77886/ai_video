...
export default function App() {
  ...
  const actionableTasks = useMemo(() => tasks.filter((task) => task.status !== 'succeeded').sort((a, b) => (taskStatusOrder[a.status] ?? 99) - (taskStatusOrder[b.status] ?? 99) || new Date(b.updated_at) - new Date(a.updated_at)), [tasks])
  ...
  const counts = { shots: visibleShots.length, assets: assets.length, videos: videos.length, tasks: actionableTasks.length }
  ...
  return (
    <div className="shell">
      <aside className="sidebar">
        ...
        {tab === 'shots' && versions.length > 0 && <label className="field dark-field"><span>分镜版本</span><select value={selectedVersion} onChange={(event) => setInputVersion(event.target.value)}>{versions.map((version) => <option key={version} value={version}>{version}</option>)}</select></label>}
      ...
      <main>
        ...
        {!projects.length ? <EmptyProject onSubmit={createProject} busy={busy} /> : !selectedProject ? <section className="empty"><h2>选择项目</h2></section> : <>
          {tab === 'shots' && <><Overview shots={visibleShots} tasks={tasks} /><Shots items={visibleShots} assets={assets} videos={videos} tasks={tasks} config={config} busy={busy} run={run} reload={reload} /></>}
          {tab === 'assets' && <Assets projectID={projectID} items={assets} shots={shots} videos={videos} tasks={tasks} config={config} busy={busy} run={run} reload={reload} />}
...
function Shots({ items, assets, videos, tasks, config, busy, run, reload }) {
  ...
  const shotImageTasks = tasks.filter((task) => task.shot_id === selected?.id && task.kind === 'image_generation')
  const hasActiveImageTask = shotImageTasks.some((task) => ['queued', 'running'].includes(task.status))
  ...
  const hasCurrentImageTask = shotImageTasks.some((task) => task.status === 'succeeded' && task.prompt?.includes(`\n\nProduction prompt:\n${selected?.prompt}\n\n`))
  const hasStaleImages = hasGeneratedImages && !hasCurrentImageTask
  ...
  return <section className="workspace shot-workspace">
    ...
    {selected ? <article className="panel shot-detail">
      ...
      <section className="generation-bar"><div><strong>分镜图</strong><small>{hasStaleImages ? '镜头脚本已修改，现有分镜图属于旧内容。' : '独立创建首帧图、末帧图、预览图；不生成视频。'}</small></div><button className="ghost" disabled={busy || !config?.openai_image_configured || hasActiveImageTask} onClick={generateImages}>{hasActiveImageTask ? '分镜图生成中' : hasStaleImages ? '重新生成分镜图' : '生成/补全分镜图'}</button></section>
...
function Assets({ projectID, items, shots, videos, tasks, config, busy, run, reload }) {
  ...
  return <section className="workspace">
    ...
    <div className="panel asset-toolbar"><div><div className="category-tabs"><button className={!kind ? 'active' : ''} onClick={() => setKind('')}>全部 {items.length}</button>{Object.entries(kindLabels).map(([value, label]) => <button className={kind === value ? 'active' : ''} key={value} onClick={() => setKind(value)}>{label} {items.filter((item) => item.kind === value).length}</button>)}</div><p className="muted">素材属于当前项目，不随分镜版本隐藏。</p></div><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索素材" /></div>
